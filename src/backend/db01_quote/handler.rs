...
// use std::env;
use crate::backend::repository_db01;
use crate::backend::DB01Quote;

#[get("/")]
pub fn all(connection: DbConn) -> Result<Json<Vec<DB01Quote>>, Status> {
    repository_db01::all(&connection)
        .map(|data| Json(data))
...
pub fn post(
    person: Json<Person>,
    connection: DbConn,
) -> Result<status::Created<Json<Person>>, Status> {
